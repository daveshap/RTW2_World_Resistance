...
Before path setup, the root loader emits `LOADER_START` to `wr2_world_resistance_bootstrap.log` and Rome II's native output when available. Every 0.1.5 line carries an opaque per-loader `load` token. `MODULE_PATH_READY`, `DIRECTOR_ROUTE_TRY`, and `DIRECTOR_ROUTE_OK` identify the selected route; route and require errors preserve a longer sanitized diagnostic. The root logger is independent of director initialization, and all of its open/write/flush/close and native-output operations are protected. If search, compilation, or director execution raises a normal Lua error, the protected import leaves the vanilla loader alive. This cannot recover from a malformed DB file or a crash inside native engine code.
...
The 0.1.5 regression suite models an initially hostile ambient `package.path`, temporary explicit routing and restoration, a director environment without a shared `events` global, the exact registry-argument identity, registration without an interface, partial setup and retry without duplicates, lazy global/cache discovery, `campaign_name("main_rome")`, first-tick initialization, and first-faction-turn-per-campaign-turn recovery. It remains simulated evidence because this build environment does not contain a runnable Rome II executable; the user's native 0.1.4 trace proves the path only through interface acquisition.
...
The root bootstrap file and detailed campaign file are separate. Installation-root `wr2_world_resistance_bootstrap.log` can record route, event, interface, and world progress before detailed telemetry exists. The detailed stream, `data/wr2_world_resistance.log`, begins only while processing a successful supported-world reconciliation at first tick or a later faction-turn fallback. `DIAGNOSTIC_SINK_READY` confirms that it opened and wrote; `DIAGNOSTIC_SINK_ERROR` explicitly reports a real sink failure. Detailed-log open, write, flush, and close are all protected; the first failure permanently disables that sink for the Lua session while the campaign director, `WORLD_STATE`, and native `out.ting` logging continue. The attached 0.1.4 run had no detailed file because it never passed world support, not because its path was proven unwritable.
...
The script does not create forces. Force spawning, especially in invalid or occupied locations, is a known source of campaign instability and turn-sequence freezes. AI army parity is pursued through legal cap access, money, recruitment slots, rank, replenishment, and low costs. Consequently, Rome II's fielded-military power bar can remain low immediately after activation; accelerated recruitment is not instant parity.
...
The separately inspected [Stable Politics: No Civil War](https://github.com/daveshap/RTW2_No_Civil_War) preset is a single DB-table fragment and contains no Lua loader, army-cap rows, or overlapping World Resistance key. It is therefore expected to coexist with 0.1.5 and cannot explain the 0.1.1 missing-interface retry, 0.1.2 module-route, 0.1.3 registry-handoff, or 0.1.4 campaign-predicate defects. The combination still needs the same live first-turn/end-turn smoke test as any two-pack setup.
...
7. **Loader ownership.** Another enabled mod that replaces `all_scripted.lua` can silently prevent one director from loading or can remove vanilla imports. A missing 0.1.5 `LOADER_START` in a writable installation is the first indicator of that condition; the `load` token, route milestones, and explicit setup outcome distinguish separate loader evaluations once this loader is selected.
